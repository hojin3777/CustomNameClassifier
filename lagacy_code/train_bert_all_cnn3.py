...
MODEL_NAME = 'bert-kor-cnn3_all' # 증강 포함
TIME_DATE = '250619_0630'
MAX_LEN = 60
BATCH_SIZE = 128
LEARNING_RATE = 1e-5
N_EPOCHS = 10
...
class TokenDataset(Dataset):
    def __init__(self, dataframe, tokenizer_pretrained, max_len, augment_prob=0.2, is_train=True):
        ...
        self.ocr_typo_map = {
            ...
            'ㅡ': 'ㅣ', 'ㅣ': 'ㅡ', 'ㅐ': 'ㅔ', 'ㅔ': 'ㅐ', 'ㄱ': 'ㅋ', 'ㅋ': 'ㄱ', 'ㄷ': 'ㅌ', 'ㅌ': 'ㄷ',
    ...
    def _introduce_ocr_errors(self, sentence, error_rate=0.15):
        chars = list(sentence)
        for i, char in enumerate(chars):
            if char in self.ocr_typo_map and random.random() < error_rate:
                chars[i] = self.ocr_typo_map[char]
        return "".join(chars)
