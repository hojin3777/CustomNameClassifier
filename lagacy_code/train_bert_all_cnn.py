...
#모델 이름 및 날짜 세팅
model_name = 'bert-kor-cnn_all' #weighted loss 추가, 병렬 커널 사용, 오버샘플링 추가
...
if __name__ == "__main__":
    ...
    # 가중치: 전체/각 클래스 샘플 수
    weights = np.array([1.0 / class_counts[cls] for cls in sorted(class_counts.keys())])
    weights = weights / weights.sum() * num_classes  # normalize
    class_weights = torch.tensor(weights, dtype=torch.float).to(device)

    # batch_size는 GPU 메모리에 맞게 조절 (16~128 추천, 64~128이 가장 무난)
    sample_weights = train_df['class'].map(lambda x: class_weights[class_to_idx[x]].item()).values
    sampler = WeightedRandomSampler(sample_weights, len(sample_weights), replacement=True)
    train_loader = DataLoader(train_data, batch_size=128, sampler=sampler, num_workers=0, pin_memory=True)
