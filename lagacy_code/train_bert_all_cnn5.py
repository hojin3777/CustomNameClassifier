...
#모델 이름 및 날짜 세팅
model_name = 'bert-kor-cnn5_all'  # 후속 레이어 제거, BERT모델에서 바로 출력 차원으로 연결
time_date = '250623_0140'
...
class BertCLSModel(nn.Module):
    def __init__(self, bert_pretrained, num_classes, dropout_rate=0.5):
        super().__init__()
        self.bert = BertModel.from_pretrained(bert_pretrained)
        self.dropout = nn.Dropout(dropout_rate)
        # BERT의 출력 차원(768)을 바로 클래스 수로 매핑
...
if __name__ == "__main__":
    ...
    bert = BertCLSModel(tokenizer_pretrained, num_classes=num_classes).to(device)
